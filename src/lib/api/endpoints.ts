...
export const adminApi = {
  ...
  updateOrderStatus: (
    id: string,
    payload: { status: AdminOrder["status"]; note?: string }
  ) =>
...
export const adminProductsApi = {
  ...
  bulkCreate: (
    products: Partial<Omit<AdminProduct, "id" | "createdAt" | "updatedAt">>[]
  ) =>
    api.post<{ created: AdminProduct[]; errors: unknown[] }>("/admin/products/bulk", {
      products,
    }),
  syncQikink: (id: string) =>
    api.post<QikinkSyncResult>(`/admin/products/${id}/sync-qikink`),
};

export const adminProfileApi = {
  update: (payload: AdminProfileUpdatePayload) =>
    api.put<AdminUser>("/admin/profile", payload),
